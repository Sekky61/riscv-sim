...
public class IssueWindowSuperBlock implements AbstractBlock {
        ...
        if (decodeAndDispatchBlock.shouldFlush()) {
            for (int i = 0; i < this.decodeAndDispatchBlock.getAfterRenameCodeList().size(); i++) {
                this.failedInstructions.push(
                        this.decodeAndDispatchBlock.getAfterRenameCodeList().get(i));
            }
            ...
            int pullCount = !decodeAndDispatchBlock.shouldStall() ? this.decodeAndDispatchBlock.getAfterRenameCodeList().size() : this.decodeAndDispatchBlock.getStalledPullCount();

            for (int i = 0; i < pullCount; i++) {
                SimCodeModel codeModel = this.decodeAndDispatchBlock.getAfterRenameCodeList().get(
                        i);
    ...
    public void simulateBackwards() {
        ...
        if (returningCodeModelList.isEmpty() && !this.failedInstructions.isEmpty() && this.failedInstructions.peek().getInstructionBulkNumber() == id) {
        ...
        this.decodeAndDispatchBlock.getAfterRenameCodeList().addAll(
                returningCodeModelList);
        this.decodeAndDispatchBlock.getAfterRenameCodeList().sort(
                SimCodeModel::compareTo);
    ...
    private void selectCorrectIssueWindow(InstructionFunctionModel instruction, SimCodeModel codeModel) {
        for (AbstractIssueWindowBlock issueWindow : this.issueWindowBlockList) {
            if (issueWindow.isCorrectInstructionType(
                    instruction.getInstructionType()) && issueWindow.isCorrectDataType(
