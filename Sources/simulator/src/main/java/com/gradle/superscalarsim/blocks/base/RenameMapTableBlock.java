...
public class RenameMapTableBlock {
    ...
    public String mapRegister(String registerName, int order) {
        ...
        this.registerMap.put(speculativeRegister,
                new RenameMapModel(registerName, order));
        this.freeList.remove(speculativeRegister);
        this.referenceMap.put(speculativeRegister, 1);
        this.registerFileBlock.getRegister(speculativeRegister).setReadiness(
    ...
    public boolean reduceReference(String speculativeRegister) {
        if (!this.registerMap.containsKey(
                speculativeRegister) || !this.referenceMap.containsKey(
                speculativeRegister)) {
            return false;
        }
        int currentRefCount = this.referenceMap.get(speculativeRegister) - 1;
    ...
    public void copyAndFreeMapping(String speculativeRegister) {
        if (isSpeculativeRegister(speculativeRegister)) {
            RenameMapModel architecturalRegister = this.registerMap.get(
                    speculativeRegister);
            this.registerFileBlock.copyAndFree(speculativeRegister,
                    architecturalRegister.getArchitecturalRegister());
    ...
    public String getMappingForRegister(String register) {
        ...
        for (Map.Entry<String, RenameMapModel> entry : this.registerMap.entrySet()) {
            if (entry.getValue().getArchitecturalRegister().equals(
                    register) && entry.getValue().getOrder() > newestOrder) {
    ...
    public void directCopyMapping(String speculativeRegister) {
        if (isSpeculativeRegister(speculativeRegister)) {
            ...
            double value = this.registerFileBlock.getRegister(
                    speculativeRegister).getValue();
            this.registerFileBlock.getRegister(
                    architecturalRegister.getArchitecturalRegister()).setValue(value);
        }
