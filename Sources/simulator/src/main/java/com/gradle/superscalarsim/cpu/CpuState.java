...
public class CpuState implements Serializable {
    ...
    public void initState(CpuConfiguration config) {
        ...
        PatternHistoryTable.PredictorType predictorType = switch (config.predictorType) {
            ...
            default -> throw new IllegalStateException(
                    "Unexpected value for predictor type: " + config.predictorType);
        };
        ...
        if (config.predictorType.equals("0bit") || config.predictorType.equals("1bit")) {
            if (!Objects.equals(config.predictorDefault, "Taken") && !Objects.equals(
                    config.predictorDefault, "Not Taken"))
                throw new IllegalStateException(
                        "Unexpected value for 0bit/1bit predictor: " + config.predictorDefault);
            ...
            defaultTaken = switch (config.predictorDefault) {
                ...
                default -> throw new IllegalStateException(
                        "Unexpected value for 2bit predictor: " + config.predictorDefault);
            };
        } else {
            throw new IllegalStateException(
                    "Unexpected predictor type: " + config.predictorType);
        }
        ...
        ReplacementPoliciesEnum replacementPoliciesEnum = switch (config.cacheReplacement) {
            ...
            default -> throw new IllegalStateException(
                    "Unexpected value for cache replacement: " + config.cacheReplacement);
        };

        boolean writeBack = true;
        if (!Objects.equals(config.storeBehavior, "write-back")) {
            throw new IllegalStateException(
                    "Unexpected value for store behavior: " + config.storeBehavior);
        }

        this.cache = new Cache(simulatedMemory, config.cacheLines, config.cacheAssoc,
                config.cacheLineSize, replacementPoliciesEnum, writeBack,
                config.addRemainingDelay, config.storeLatency, config.loadLatency,
                config.laneReplacementDelay, this.cacheStatisticsCounter);

        this.memoryModel = new MemoryModel(cache, cacheStatisticsCounter);
        this.loadStoreInterpreter = new CodeLoadStoreInterpreter(initLoader, memoryModel,
                unifiedRegisterFileBlock);

        this.instructionFetchBlock = new InstructionFetchBlock(simCodeModelAllocator,
                codeParser, gShareUnit, branchTargetBuffer);
        instructionFetchBlock.setNumberOfWays(config.fetchWidth);

        this.decodeAndDispatchBlock = new DecodeAndDispatchBlock(simCodeModelAllocator,
                instructionFetchBlock, renameMapTableBlock, globalHistoryRegister,
                branchTargetBuffer, codeParser, initLoader);
        this.reorderBufferBlock = new ReorderBufferBlock(unifiedRegisterFileBlock,
                renameMapTableBlock, decodeAndDispatchBlock, gShareUnit,
        ...
        this.issueWindowSuperBlock = new IssueWindowSuperBlock(decodeAndDispatchBlock,
                initLoader);
        this.arithmeticInterpreter = new CodeArithmeticInterpreter(initLoader,
                precedingTable, unifiedRegisterFileBlock);
        this.branchInterpreter = new CodeBranchInterpreter(codeParser, initLoader,
                unifiedRegisterFileBlock);

        this.storeBufferBlock = new StoreBufferBlock(loadStoreInterpreter,
                decodeAndDispatchBlock, unifiedRegisterFileBlock, initLoader,
                reorderBufferBlock);
        storeBufferBlock.setBufferSize(config.sbSize);

        this.loadBufferBlock = new LoadBufferBlock(loadStoreInterpreter, storeBufferBlock,
                decodeAndDispatchBlock, unifiedRegisterFileBlock, initLoader,
        ...
        this.aluIssueWindowBlock = new AluIssueWindowBlock(initLoader,
                unifiedRegisterFileBlock, precedingTable);
        this.branchIssueWindowBlock = new BranchIssueWindowBlock(initLoader,
                unifiedRegisterFileBlock);
        this.fpIssueWindowBlock = new FpIssueWindowBlock(initLoader,
                unifiedRegisterFileBlock, precedingTable);
        this.loadStoreIssueWindowBlock = new LoadStoreIssueWindowBlock(initLoader,
                unifiedRegisterFileBlock);
        ...
        for (CpuConfiguration.FUnit fu : config.fUnits) {
            switch (fu.fuType) {
                case "FX" -> {
                    ArithmeticFunctionUnitBlock functionBlock = new ArithmeticFunctionUnitBlock(
                            reorderBufferBlock, fu.latency, fpIssueWindowBlock,
                            fu.operations);
                ...
                case "L/S" -> {
                    LoadStoreFunctionUnit loadStoreFunctionUnit = new LoadStoreFunctionUnit(
                            reorderBufferBlock, fu.latency, loadStoreIssueWindowBlock,
                            loadBufferBlock, storeBufferBlock, loadStoreInterpreter);
                    this.loadStoreIssueWindowBlock.setFunctionUnitBlock(
                            loadStoreFunctionUnit);
                    this.loadStoreFunctionUnits.add(loadStoreFunctionUnit);
                }
                case "Branch" -> {
                    BranchFunctionUnitBlock branchFunctionUnitBlock = new BranchFunctionUnitBlock(
                            reorderBufferBlock, branchIssueWindowBlock, fu.latency);
                    branchFunctionUnitBlock.addBranchInterpreter(branchInterpreter);
                    branchFunctionUnitBlock.addRegisterFileBlock(
                            unifiedRegisterFileBlock);
                    this.branchIssueWindowBlock.setFunctionUnitBlock(
                            branchFunctionUnitBlock);
                    this.branchFunctionUnitBlocks.add(branchFunctionUnitBlock);
                }
                case "Memory" -> {
                    MemoryAccessUnit memoryAccessUnit = new MemoryAccessUnit(
                            reorderBufferBlock, fu.latency, loadStoreIssueWindowBlock,
                            loadBufferBlock, storeBufferBlock, loadStoreInterpreter,
                            unifiedRegisterFileBlock);
    ...
    public void step() {
        ...
        if (loadBufferBlock.getLoadQueueFirst() == null || storeBufferBlock.getStoreQueueFirst() == null || loadBufferBlock.getLoadQueueFirst().getId() < storeBufferBlock.getStoreQueueFirst().getId()) {
    ...
    public void stepBack() {
        ...
        // Run all FUs
        arithmeticFunctionUnitBlocks.forEach(
                ArithmeticFunctionUnitBlock::simulateBackwards);
