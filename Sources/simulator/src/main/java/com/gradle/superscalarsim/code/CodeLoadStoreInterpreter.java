...
public class CodeLoadStoreInterpreter {
     ...
     */
    public CodeLoadStoreInterpreter(final InitLoader initLoader, final MemoryModel memoryModel, final UnifiedRegisterFileBlock registerFileBlock) {
    ...
    public long interpretAddress(final IInputCodeModel parsedCode) {
        ...
        String[] interpretableAsParams = instruction.getInterpretableAs().replace(";",
                "").split(" ");

        InputCodeArgument addressRegister = parsedCode.getArgumentByName(
                interpretableAsParams[3]);
        InputCodeArgument offsetImmediate = parsedCode.getArgumentByName(
                interpretableAsParams[4]);

        long address = Double.valueOf(
                getValueFromOperand(Objects.requireNonNull(addressRegister).getValue(),
                        instruction.getInputDataType())).longValue();
    ...
    public Pair<Integer, Double> interpretInstruction(final SimCodeModel parsedCode, int currentCycle) {
        ...
        switch (interpretableAsParams[0]) {
            case "load" -> {
                ...
                return processLoadOperation(loadParams[0],
                        loadParams[1].equals("unsigned"), destination, address, offset,
                        parsedCode.getId(), currentCycle);
            }
            case "store" -> {
                double sourceValue = Double.valueOf(getValueFromOperand(
                        Objects.requireNonNull(loadStoreRegister).getValue(),
                        instruction.getOutputDataType())).longValue();
                return processStoreOperation(interpretableAsParams[1], sourceValue,
                        address, offset, parsedCode.getId(), currentCycle);
            }
            default -> throw new IllegalStateException(
                    "Unexpected value: " + interpretableAsParams[0]);
        }
     ...
     */
    private Pair<Integer, Double> processLoadOperation(String size, boolean isUnsigned, String destination, long address, long offset, int id, int currentCycle) {
        long effectiveAddress = address + offset;
        int numberOfBytes = sizeToNumberOfBytes(size);
        Pair<Integer, Long> loadedData = memoryModel.load(effectiveAddress, numberOfBytes,
                id, currentCycle);
        ...
        if (registerModel == null) {
            registerModel = this.registerFileBlock.getRegisterList(
                    DataTypeEnum.kSpeculative).stream().filter(
                    register -> register.getName().equals(
                            destination)).findFirst().orElse(null);
        }
        ...
        double result = switch (numberOfBytes) {
            ...
            case 8 ->
                    size.equals("double") ? byteBuffer.getDouble() : byteBuffer.getLong();
            default -> Double.NaN;
        };
     ...
     */
    private Pair<Integer, Double> processStoreOperation(String size, double sourceValue, long address, long offset, int id, int currentCycle) {
        long effectiveAddress = address + offset;
        int numberOfBytes = sizeToNumberOfBytes(size);
        ...
        return new Pair<>(
                memoryModel.store(effectiveAddress, storeValue, numberOfBytes, id,
                        currentCycle), sourceValue);
    ...
    private double getValueFromOperand(String operand, DataTypeEnum dataType) {
        ...
        for (DataTypeEnum possibleDataType : dataTypeEnums) {
            registerModel = this.registerFileBlock.getRegisterList(
                    possibleDataType).stream().filter(
                    register -> register.getName().equals(operand)).findFirst().orElse(
                    null);
            if (registerModel != null) {
                break;
            }
