...
public class LoadBufferBlock implements AbstractBlock {
     ...
     * @brief Constructor
     */
    public LoadBufferBlock(CodeLoadStoreInterpreter loadStoreInterpreter, StoreBufferBlock storeBufferBlock, DecodeAndDispatchBlock decodeAndDispatchBlock, UnifiedRegisterFileBlock registerFileBlock, InitLoader initLoader, ReorderBufferBlock reorderBufferBlock, InstructionFetchBlock instructionFetchBlock) {
    ...
    public void releaseLoadFirst() {
        ...
        } else throw new RuntimeException("Release store when load queue is empty");
    ...
    private void pullLoadInstructionsFromDecode() {
        decodeAndDispatchBlock.getAfterRenameCodeList().forEach(codeModel -> {
            ...
            InputCodeArgument argument = codeModel.getArgumentByName("rd");
            this.loadMap.put(codeModel.getId(),
                    new LoadBufferItem(Objects.requireNonNull(argument).getValue()));
    ...
    private void selectLoadForDataAccess() {
        ...
        for (SimCodeModel simCodeModel : this.loadQueue) {
            ...
            if (isAvailableForMA) {
                ...
                } else if (possibleCodeModel != null && codeModel.compareTo(
                        possibleCodeModel) > 0) {
                    codeModel = possibleCodeModel;
                }
            }
