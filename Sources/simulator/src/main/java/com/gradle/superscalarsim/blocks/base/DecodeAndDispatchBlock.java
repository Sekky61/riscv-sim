...
public class DecodeAndDispatchBlock implements AbstractBlock {
     ...
     * @brief Constructor
     */
    public DecodeAndDispatchBlock(SimCodeModelAllocator simCodeModelAllocator, InstructionFetchBlock instructionFetchBlock, RenameMapTableBlock renameMapTableBlock, GlobalHistoryRegister globalHistoryRegister, BranchTargetBuffer branchTargetBuffer, CodeParser codeParser, InitLoader initLoader) {
    ...
    public void simulate() {
        ...
        if (stallFlag) {
            ...
            int fetchCount = Math.min(
                    (int) this.instructionFetchBlock.getFetchedCode().stream().filter(
                            code -> !code.getInstructionName().equals("nop")).count(),
            ...
            for (int i = 0; i < fetchCount; i++) {
                SimCodeModel codeModel = this.instructionFetchBlock.getFetchedCode().get(
                        i);
        ...
        this.beforeRenameCodeList.removeIf(
                code -> code.getInstructionName().equals("nop"));
        this.beforeRenameCodeList.removeIf(
                code -> code.getInstructionName().equals("label"));
    ...
    private void renameDestinationRegister(SimCodeModel simCodeModel) {
        ...
        // Rename
        destinationArgument.setValue(
                renameMapTableBlock.mapRegister(destinationArgument.getValue(),
                        simCodeModel.getId()));
    ...
    private void processBranchInstruction(final SimCodeModel codeModel, int position, int modelId) {
        ...
        if (jumpBad || branchBad) {
            ...
            this.branchTargetBuffer.setEntry(instructionPosition, codeModel, realTarget,
                    modelId, -1);
    ...
    private int calculateRealBranchAddress(final SimCodeModel codeModel) {
        ...
        if (immediateArgument == null) {
            throw new RuntimeException(
                    "Branch instruction does not have immediate argument");
        }
        ...
        if (labelOffset == -1) {
            ...
            return codeModel.getSavedPc() + Integer.parseInt(
                    immediateArgument.getValue());
        }
        // Jump after label
