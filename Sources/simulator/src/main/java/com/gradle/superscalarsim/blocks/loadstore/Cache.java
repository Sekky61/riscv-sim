...
public class Cache
{
    ...
    public Cache(SimulatedMemory memory,
                 int numberOfLines, int associativity, int lineSize,
                 ReplacementPoliciesEnum replacementPolicy,
                 boolean writeBack, boolean addRemainingDelayToStore,
                 int storeDelay, int loadDelay, int lineReplacementDelay,
        ...
        this.replacementPolicy = ReplacementPolicyModel.getReplacementPolicyModel(replacementPolicy, numberOfLines, associativity);
        this.memory = memory;
     ...
     */
    public Cache(CacheStatisticsCounter cacheStatisticsCounter, int numberOfLines, int associativity, int lineSize, CacheLineModel[][] cache, ReplacementPolicyModel replacementPolicy, SimulatedMemory memory, boolean writeBack, Stack<CacheLineModel> cacheLineHistory, Stack<Integer> cacheIdHistory, Stack<CacheAccess> lastAccess, int storeDelay, int loadDelay, int lineReplacementDelay, boolean addRemainingDelayToStore, ReplacementPoliciesEnum replacementPolicyType, int cycleEndOfReplacement) {
        this.cacheStatisticsCounter = cacheStatisticsCounter;
        this.numberOfLines = numberOfLines;
        this.associativity = associativity;
        this.lineSize = lineSize;
    ...
    private int remainingDelay(boolean isHit, CacheAccess matchedAccess, int currentCycle)
    {
        ...
        else
        {
            ...
            cycleEndOfReplacement = (cycleEndOfReplacement <= currentCycle)
                    ? currentCycle + lineReplacementDelay
    ...
    private int computeRemainingDelay(boolean isHit, boolean isStore, int currentCycle, long tag, int index)
    {
        ...
        for (int i = lastAccess.size()-1; i>0; i--)
        {
            ...
            int tmpAccessDelayEnd = Math.max(tmpAccess.getClockCycle(), tmpAccess.getEndOfReplacement())+tmpAccess.getDelay();
            ...
            else if (tmpAccess.getClockCycle() < currentCycle)
            {
                ...
                //If the tag doesn't match check if tmpAccess was misaligned over last cache line
                        ...
                        index == 0 &&
                        tmpAccess.getTag()-1 == tag &&
                        tmpAccess.getIndex() == numberOfLines/associativity-1 &&
                        tmpAccess.getCacheIndex().length > 1
                )
                {
                    int delay = remainingDelay(isHit, tmpAccess, currentCycle);
                    lastAccess.get(lastAccess.size() - 1).setDelay(lastAccess.get(lastAccess.size() - 1).getDelay()+delay);
    ...
    {
        return numberOfLines % associativity == 0 &&
               lineSize % 4 == 0 &&
               numberOfLines % 2 == 0;
    }
    ...
    private Pair <Integer, Long> getDataFromLines(CacheLineModel line, long address, int size, int id, int currentCycle)
    {
        ...
        else
        {
            ...
            Pair<Integer, Long> tmpReturnVal = getData(address+sizeInsideCurrentLine, size-(int)sizeInsideCurrentLine, id, currentCycle);
            return new Pair<>(tmpReturnVal.getFirst() ,(tmpReturnVal.getSecond()<<(size-sizeInsideCurrentLine)*8) | bottomPart);
        }
    }
    ...
    public Pair<Integer, Long> getData(long address, int size, int id, int currentCycle)
    {
        ...
        if (lastAccess.peek().getId() != id)
        {
            ...
            this.lastAccess.add(new CacheAccess(currentCycle, cycleEndOfReplacement, id, new Boolean[0], false, splittedAddress, 0, new Integer[0], new Integer[0]));
        }
        ...
        for (int i = 0; i < associativity; i++)
        {
            ...
            if (line.getTag() == splittedAddress.getFirst() && line.isValid())
            {
                ...
                lastAccess.peek().addLineAccess(true, splittedAddress.getSecond()*associativity+i, splittedAddress.getThird());
                Pair<Integer, Long> tmpReturnVal = getDataFromLines(line, address, size, id, currentCycle);
                return new Pair<>(tmpReturnVal.getFirst()+loadDelay+computeRemainingDelay(
                                       true,
                                       false,
                                       currentCycle,
            ...
            if (cache[splittedAddress.getSecond()][selectedLine].isDirty())
            {
                ...
                for (int i = 0; i < lineSize; i++)
                {
                    memory.insertIntoMemory(cache[splittedAddress.getSecond()][selectedLine].getBaseAddress() + i,
                            (byte) cache[splittedAddress.getSecond()][selectedLine].getData(i, 1),
                            id
                    );
                }
            }
        ...
        lastAccess.peek().addLineAccess(false, splittedAddress.getSecond()*associativity+selectedLine, splittedAddress.getThird());
        Pair<Integer, Long> tmpReturnVal = getDataFromLines(cache[splittedAddress.getSecond()][selectedLine], address, size, id, currentCycle);
        return new Pair<>(tmpReturnVal.getFirst()+loadDelay+computeRemainingDelay(
                              false,
                              false,
                              currentCycle,
    ...
    private int setDataToLines(CacheLineModel line, long address, long data, int size, int id, int currentCycle)
    {
        ...
        else
        {
            ...
            return storeData(address+sizeInsideCurrentLine, data>>sizeInsideCurrentLine*8,size-(int)sizeInsideCurrentLine, id, currentCycle);
        }
    }
    ...
    public int storeData(long address, long data, int size, int id, int currentCycle)
    {
        ...
        {
            cacheStatisticsCounter.incrementAccesses();
            lastAccess.add(new CacheAccess(currentCycle, cycleEndOfReplacement, id, new Boolean[0], true, splittedAddress, 0, new Integer[0], new Integer[0]));
        }
        ...
        int tmpReturn = setDataToLines(cache[splittedAddress.getSecond()][selectedLine], address, data, size, id, currentCycle);
        return storeDelay + tmpReturn +
                    computeRemainingDelay(
                        false,
                        true,
    ...
    public CacheAccess getLastAccess(){ return lastAccess.peek(); }
     ...
     */
    public int getStoreDelay(){ return storeDelay; }
     ...
     */
    public int getLineReplacementDelay(){ return lineReplacementDelay; }

    /**
     * @brief Gets if line replacement delay should be added to store
     */
    public boolean getAddRemainingDelayToStore(){ return addRemainingDelayToStore; }

    /**
     * @brief Gets current replacement policy
     */
    public ReplacementPoliciesEnum getReplacementPolicyType(){ return replacementPolicyType; }

    /**
     * @brief Is store behaviour set to write-back or write-through?
     */
    public boolean isWriteBack(){ return writeBack; }
}
