...
public class CodeBranchInterpreter {
     ...
     * @brief Constructor
     */
    public CodeBranchInterpreter(final CodeParser codeParser, final InitLoader initLoader, final UnifiedRegisterFileBlock registerFileBlock) {
    ...
    public OptionalInt interpretInstruction(final IInputCodeModel parsedCode, int instructionPosition) {
        ...
        if (splitInterpretableAs[0].startsWith("jump")) {
            ...
            if (jumpExpr.equals("imm")) {
                ...
                int rs1 = (int) getValueFromOperand(
                        parsedCode.getArgumentByName("rs1").getValue(),
                        instruction.getInputDataType());
                InputCodeArgument immArgument = parsedCode.getArgumentByName("imm");
                int jumpOffset = rs1 + Integer.parseInt(
                        Objects.requireNonNull(immArgument).getValue());
                return OptionalInt.of(jumpOffset);
            } else {
                throw new IllegalArgumentException(
                        "InterpretableAs in instruction " + instruction.getName() + " is not valid");
            }
        } else {
            // example: signed:rs1 != rs2
            boolean isUnsigned = splitInterpretableAs[0].equals("unsigned");
            boolean expr = interpretExpression(splitInterpretableAs[1], parsedCode,
                    isUnsigned, instruction.getInputDataType());
            if (!expr) {
        ...
        if (labelArgument != null) {
            ...
            int returnPosition = (int) getValueFromOperand(
                    Objects.requireNonNull(jumpArgument).getValue(),
                    instruction.getInputDataType());
     ...
     */
    private boolean interpretExpression(final String expression, final IInputCodeModel parsedCode, final boolean isUnsigned, final DataTypeEnum inputDataType) {
        String temp = "";
        ...
        InputCodeArgument rightOperandArgument = parsedCode.getArgumentByName(
                rightOperand);

        return evaluateExpression(Objects.requireNonNull(leftOperandArgument).getValue(),
                Objects.requireNonNull(rightOperandArgument).getValue(), operator,
                isUnsigned, inputDataType);
     ...
     */
    private boolean evaluateExpression(final String leftOperand, final String rightOperand, final String operator, final boolean isUnsigned, final DataTypeEnum inputDataType) {
