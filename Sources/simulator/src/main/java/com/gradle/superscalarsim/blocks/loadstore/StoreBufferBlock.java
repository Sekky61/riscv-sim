...
public class StoreBufferBlock implements AbstractBlock {
     ...
     * @brief Constructor
     */
    public StoreBufferBlock(CodeLoadStoreInterpreter loadStoreInterpreter, DecodeAndDispatchBlock decodeAndDispatchBlock, UnifiedRegisterFileBlock registerFileBlock, InitLoader initLoader, ReorderBufferBlock reorderBufferBlock) {
    ...
    public void releaseStoreFirst() {
        ...
        } else throw new RuntimeException("Release store when store queue is empty");
    ...
    private void pullStoreInstructionsFromDecode() {
        decodeAndDispatchBlock.getAfterRenameCodeList().forEach(codeModel -> {
            if (isInstructionStore(codeModel)) {
                if (isBufferFull(1) || !this.reorderBufferBlock.getFlagsMap().containsKey(
                        codeModel.getId())) {
                    return;
                }
                this.storeQueue.add(codeModel);
                ...
                this.storeMap.put(codeModel.getId(),
                        new StoreBufferItem(Objects.requireNonNull(argument).getValue(),
                                codeModel.getId()));
            }
        });
    ...
    private void selectStoreForDataAccess() {
        ...
        for (SimCodeModel simCodeModel : this.storeQueue) {
            ...
            if (item.getAddress() == -1) break;

            boolean isAvailableForMA = item.getAddress() != -1 && !item.isAccessingMemory() && item.getAccessingMemoryId() == -1 && item.isSourceReady() && !reorderBufferBlock.getFlagsMap().get(
                    simCodeModel.getId()).isSpeculative();
            if (isAvailableForMA) for (SimCodeModel previousStore : this.storeQueue) {
                //Check if we haven't reached current statement
                if (simCodeModel.getId() == previousStore.getId()) break;
                else if ((this.storeMap.get(
                        previousStore.getId()).getAddress() & ~3L) == (item.getAddress() & ~3L)) {
        ...
        for (SimCodeModel simCodeModel : this.storeQueue) {
            if (simCodeModel.hasFailed()) {
                ...
                if (this.storeMap.get(simCodeModel.getId()).isAccessingMemory()) {
                    this.memoryAccessUnitList.forEach(
                            ma -> ma.tryRemoveCodeModel(simCodeModel));
                }
    ...
    public boolean isInstructionStore(SimCodeModel codeModel) {
        ...
        return instruction != null && instruction.getInterpretableAs().startsWith(
                "store");
