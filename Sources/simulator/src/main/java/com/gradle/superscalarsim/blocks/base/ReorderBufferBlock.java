...
public class ReorderBufferBlock implements AbstractBlock {
     ...
     * @brief Constructor
     */
    public ReorderBufferBlock(UnifiedRegisterFileBlock registerFileBlock, RenameMapTableBlock renameMapTableBlock, DecodeAndDispatchBlock decodeAndDispatchBlock, GShareUnit gShareUnit, BranchTargetBuffer branchTargetBuffer, InstructionFetchBlock instructionFetchBlock, StatisticsCounter statisticsCounter, ReorderBufferState state) {
    ...
    public void simulate() {
        ...
        for (SimCodeModel currentInstruction : this.state.reorderQueue) {
            ReorderFlags currentReorderFlags = this.state.flagsMap.get(
        ...
        do {
            ...
            if (currentReorderFlags.isReadyToBeCommitted()) {
                ...
                this.state.flagsStack.push(
                        this.state.flagsMap.get(currentInstruction.getId()));
    ...
    public void simulateBackwards() {
        ...
        while (!this.state.releaseStack.empty() && this.state.releaseStack.peek().getCommitId() == this.state.commitId) {
            ...
            if (currentInstruction.getInstructionTypeEnum() == InstructionTypeEnum.kJumpbranch) {
                ...
                this.gShareUnit.getGlobalHistoryRegister().revertToHistory(
                        currentInstruction.getId());
                if (!currentInstruction.hasFailed()) {
                    this.gShareUnit.getPredictorFromOld(pcValue,
                            currentInstruction.getId()).predictBackwards();
                }
        ...
        this.state.speculativePulls = false;
        this.state.reorderQueue.stream().sorted().reduce(
                (first, second) -> second).ifPresent(codeModel -> {
            ReorderFlags codeModelFlags = state.flagsMap.get(codeModel.getId());
    ...
    private void pullNewDecodedInstructions() {
        if (!this.decodeAndDispatchBlock.shouldFlush()) {
            ...
            for (SimCodeModel codeModel : this.decodeAndDispatchBlock.getAfterRenameCodeList()) {
                ...
                this.state.flagsMap.put(codeModel.getId(),
    ...
    private boolean checkIfInstructionsHaveRoom(SimCodeModel codeModel) {
        ...
        return !this.isBufferFull(
                allInstructionCount) && !this.loadBufferBlock.isBufferFull(
    ...
    private void resetAssignedRegisters() {
        while (!this.state.preCommitModelStack.empty() && this.state.preCommitModelStack.peek().getId() == this.state.commitId) {
            ...
            if (destinationRegister.getSpeculRegister().isEmpty()) {
                ...
                this.renameMapTableBlock.mapRegister(
                        destinationRegister.getArchRegister(),
                        destinationRegister.getSpeculRegister(),
    ...
    private void processCommittableInstruction(SimCodeModel codeModel) {
        if (codeModel.getInstructionTypeEnum() == InstructionTypeEnum.kJumpbranch) {
            ...
            if (prediction == branchActuallyTaken) {
                ...
                this.gShareUnit.getPredictorFromOld(pc,
                        codeModel.getId()).upTheProbability();
                this.gShareUnit.getGlobalHistoryRegister().removeHistoryValue(
                        codeModel.getId());
                ...
                this.gShareUnit.getPredictorFromOld(pc,
                        codeModel.getId()).downTheProbability();
                this.branchTargetBuffer.setEntry(pc, codeModel, resultPc, -1,
                        state.commitId);
        ...
        if (destinationArgument != null) {
            ...
            saveRegisterValue(
                    renameMapTableBlock.getMapping(destinationArgument.getValue()),
                    codeModel.getId());
        ...
        codeModel.getArguments().stream().filter(
                argument -> argument.getName().startsWith("r")).forEach(argument -> {
            saveRegisterValue(argument.getValue(), codeModel.getId());
    ...
    public void invalidateInstructions(SimCodeModel firstInvalidInstruction) {
        ...
        this.decodeAndDispatchBlock.getAfterRenameCodeList().forEach(
                simCodeModel -> simCodeModel.getArguments().stream().filter(
                        argument -> argument.getName().startsWith("r")).forEach(
                        argument -> {
        ...
        this.state.speculativePulls = polledInstructions.isEmpty() ? false : this.state.flagsMap.get(
                polledInstructions.get(
                        polledInstructions.size() - 1).getId()).isSpeculative();
    ...
    private void resetSpeculativeFlags(int codeModelId) {
        this.state.flagsMap.forEach(
                (integer, reorderFlags) -> reorderFlags.setSpeculative(
                        integer != codeModelId));
        ...
        for (SimCodeModel currentInstruction : this.state.reorderQueue) {
            ...
            if (currentReorderFlags.isReadyToBeRemoved()) {
                ...
                currentInstruction.getArguments().stream().filter(
                        argument -> argument.getName().startsWith("r")).forEach(
                        argument -> {
                            saveRegisterValue(argument.getValue(),
                                    currentInstruction.getId());
                            if (renameMapTableBlock.reduceReference(
                                    argument.getValue())) {
                                renameMapTableBlock.freeMapping(argument.getValue());
